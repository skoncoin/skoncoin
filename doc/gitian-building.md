...
*Setup instructions for a Gitian build of Skoncoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Skoncoin
...
- [Building Skoncoin Core](#building-skoncoin-core)
...
Clone the git repositories for Skoncoin Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/skoncoin-official/skoncoin
```
...
in the Skoncoin Core repository under 'Fetch and build inputs' to install sources which require
...
To build Skoncoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Skoncoin Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/skoncoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/skoncoin-official/skoncoin
...
For example:
```bash
URL=https://github.com/crowning-/skoncoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit skoncoin=${COMMIT} --url skoncoin=${URL} ../skoncoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit skoncoin=${COMMIT} --url skoncoin=${URL} ../skoncoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit skoncoin=${COMMIT} --url skoncoin=${URL} ../skoncoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the skoncoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../skoncoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/skoncoin.git
SIGPATH=/some/root/path/skoncoin-detached-sigs.git

./bin/gbuild --url skoncoin=${BTCPATH},signature=${SIGPATH} ../skoncoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/skoncoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/skoncoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/skoncoin-osx-build.assert
```
...
[skoncoin/gitian.sigs](https://github.com/skoncoin-official/gitian.sigs/) repository, or if that's not possible to create a pull
request.
