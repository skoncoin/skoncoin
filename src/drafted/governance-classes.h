 ...
 *
 *   // network
 *   CSkoncoinNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.skoncoin.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CSkoncoinNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CSkoncoinNetwork(UniValue objIn)
    {
...
// // can be under: SkoncoinNetwork
// //   -- signature requirements : Key1(User)
// class CSkoncoinNetworkVariable : public CGovernanceObject
// {
// private:
