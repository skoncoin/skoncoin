...
const char * const BITCOIN_CONF_FILENAME = "skoncoin.conf";
const char * const BITCOIN_PID_FILENAME = "skoncoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "skoncoin" is a composite category enabling all Skoncoin-related debug output
            if(ptrCategory->count(string("skoncoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SkoncoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SkoncoinCore
    // Mac: ~/Library/Application Support/SkoncoinCore
    // Unix: ~/.skoncoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SkoncoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SkoncoinCore";
#else
    // Unix
    return pathRet / ".skoncoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty skoncoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override skoncoin.conf
